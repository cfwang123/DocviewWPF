...
Windows 多标签文档应用（.NET Framework 4.8 WPF），支持 **PDF / DOCX / XLSX / CSV / TXT / Markdown / 图片**，并提供 **浏览器** 与 **命令行** 标签。核心能力：快速打开与 PDF 连续阅读、**PDF 专业编辑** 与 **标注**、大表 **XLSX** 虚拟网格与简单编辑、DOCX 结构阅读、**TXT/MD 预览与编辑**（Mermaid、语法高亮、工程模式）、Chrome 式标签、文件夹工作区、书签栏、单实例、会话恢复、自动更新、四语界面（中/英/日/韩）。
...
- **多格式同窗** — 同一应用内多标签打开 **PDF / DOCX / XLSX / CSV·TSV / TXT·代码 / MD / 图片**；另支持 **浏览器**（Edge WebView2）与 **命令行**（ConPTY + VT，cmd/PowerShell，可跑 TUI）。
...
生成：`release/DocviewWPF_x.x.x.7z`（版本取自 csproj `<Version>`；`release/` 已 gitignore，不上传）。
...
DocviewWPF/
  ...
  Viewers/        # PDF / DOCX / XLSX / CSV / TXT / MD / 图片 / 浏览器 / 命令行 与 PDF 编辑·标注
